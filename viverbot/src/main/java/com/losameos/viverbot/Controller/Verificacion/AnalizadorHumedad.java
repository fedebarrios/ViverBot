...
public class AnalizadorHumedad extends Analizador {
	...
	public void analizar(Humedad valorActual, Ubicacion u) {
		...
		if (this.verificarRango(valorActual)) {
			...
			System.out
					.println("la humedad actual es: " + this.estado.getHumedad().getValor() + " y ademas es bonita :v");
		} else {
			System.out.println(
					"la humedad actual es: " + this.estado.getHumedad().getValor() + " y ademas es bonita >:v");
